...
pub fn check_config(config: &LdxConfig) {
    ...
    for (key, flag) in &config.flags {
        ...
        if let Some(prior) = seen_shorts.get(flag.short.as_str()) {
            println!(
                ...
                "WARN".yellow().bold(), flag.short, key, prior
            );
            warnings += 1;
        } else {
        ...
        if let Some(prior) = seen_longs.get(flag.long.as_str()) {
            println!(
                ...
                "WARN".yellow().bold(), flag.long, key, prior
            );
            warnings += 1;
        } else {
    ...
    if config.custom.is_empty() {
        ...
        println!("  {} {} defined", "Custom flags:".bold(), config.custom.len());
        for (key, flag) in &config.custom {
            if flag.action.is_none() {
                println!("    {} [custom.{}] has no 'action' field", "WARN".yellow().bold(), key);
                warnings += 1;
            }
            if flag.target.is_none() {
                println!("    {} [custom.{}] has no 'target' field", "WARN".yellow().bold(), key);
                warnings += 1;
            }
            ...
            println!(
                ...
                target.cyan(),
                if value.is_empty() { String::new() } else { format!("= {}", value.bright_white()) }
            );
        }
    }
    ...
    let missing: Vec<&str> = default_targets
        ...
        .filter(|t| {
            *t != "help"
                && !config.flags.values().any(|f| f.target.as_deref() == Some(t))
        })
        .collect();

    if !missing.is_empty() {
        println!();
        println!("  {} flags missing vs defaults (run --sync to restore):", "Missing:".yellow().bold());
        for m in &missing {
            println!("    • {}", m);
        }
    ...
    if warnings == 0 {
        ...
        println!("  {} {} warning{} found", "!!".yellow().bold(), warnings, if warnings == 1 { "" } else { "s" });
    }
...
pub fn sync_config() -> Result<()> {
    ...
    let user_targets: Vec<String> = user.flags.values().filter_map(|f| f.target.clone()).collect();
    ...
    for (key, flag) in &defaults.flags {
        if user_keys.contains(&key) { continue; }
        if let Some(target) = &flag.target
            && user_targets.iter().any(|t| t == target)
        {
            continue;
        }
        let target = match &flag.target { Some(t) => t, None => continue };
        let action = flag.action.as_deref().unwrap_or("set_boolean");
        appended.push_str(&format!(
            "\n[flags.{}]\nshort = \"{}\"\nlong = \"{}\"\ndescription = \"{}\"\nos = \"{}\"\naction = \"{}\"\ntarget = \"{}\"\n",
            key, flag.short, flag.long, flag.description, flag.os, action, target
        ));
        println!("  {} added [flags.{}]  (-{} / --{})", "+".green().bold(), key, flag.short, flag.long);
        added += 1;
    }

    if added == 0 {
        println!("{}", "Config is already up to date — nothing to sync.".green());
        return Ok(());
    }

    let new_contents = format!("{}\n{}", contents.trim_end(), appended);
    std::fs::write(&path, new_contents)?;
    println!("{} {} flag{} added.", "Synced:".green().bold(), added, if added == 1 { "" } else { "s" });
    Ok(())
}
...
pub fn reset_config() -> Result<()> {
    ...
    let meta_block: String = {
        ...
        for line in user_contents.lines() {
            if line.starts_with("[meta]") { in_meta = true; }
            if in_meta {
                if line.starts_with('[') && !line.starts_with("[meta]") { break; }
                lines.push(line);
            }
        }
        lines.join("\n")
    ...
    for (key, flag) in &defaults.flags {
        let action = flag.action.as_deref().unwrap_or("set_boolean");
        let target_line = flag.target.as_deref()
            .map(|t| format!("\ntarget = \"{}\"", t))
            .unwrap_or_default();
        out.push_str(&format!(
            "[flags.{}]\nshort = \"{}\"\nlong = \"{}\"\ndescription = \"{}\"\nos = \"{}\"\naction = \"{}\"{}\n\n",
    ...
    for (key, flag) in &user.custom {
        ...
        if let Some(action) = &flag.action { out.push_str(&format!("action = \"{}\"\n", action)); }
        if let Some(target) = &flag.target { out.push_str(&format!("target = \"{}\"\n", target)); }
        if let Some(value) = &flag.value { out.push_str(&format!("value = \"{}\"\n", value)); }
        out.push('\n');
    }
    ...
    println!(
        ...
        "Done:".green().bold(),
        user.aliases.len(), if user.aliases.len() == 1 { "" } else { "es" },
        user.custom.len(), if user.custom.len() == 1 { "" } else { "s" }
    );
    Ok(())
}
